...
async def warm_up_proc(callback: CallbackQuery, state: FSMContext):
    ...
    match callback.data:
        ...
        case "ex_2":
            ...
            message_two = await bot.send_message(chat_id=callback.from_user.id, text="Вибрация-танец\n\n1) Встаньте и закройте глаза, под музыку начинайте искать вибрацию, мелкую тряску. Она начинается от пола, идет по ногам и проходит через все тело, постепенно подключая колени, таз, плечи, руки и голову.\n2) Через пару минут начинайте увеличивать амплитуду движения, при этом сохраняйте тот же образ движения.\n3) Далее вибрация постепенно становится волной, которая проходит через все тело.\n4)Замедлите свои движения и постепенно открывайте глаза.\n\nПродолжительность – 10-15 минут ")
...
async def global_cb_proc(callback: CallbackQuery, state: FSMContext):
    ...
    if callback.data == "warm_up":
        ...
        message_one = await bot.send_message(chat_id=callback.from_user.id, text="Музыкальная составляющая терапии очень важна. Предлагаю вам послушать эти мелодии и найти те, которые вызовут у вас наибольший эмоциональный отклик.")
